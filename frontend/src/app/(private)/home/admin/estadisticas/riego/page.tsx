...
// 🚀 URL DEL BACKEND DESPLEGADO (Hardcodeado según tu código)
// Nota: En un entorno de producción, se recomienda usar variables de entorno como BACKEND_URL.
const SOCKET_URL = process.env.NEXT_PUBLIC_BACKEND_URL?.replace('/api', '') || 'http://localhost:4000';

// 🟢🟢🟢 COMPONENTE CARD (Estilo azul) 🟢🟢🟢
function Card({
  title,
  value,
  icon,
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // Se asume que el backend envía la data con unidades
  ...
  return (
    <div className="pl-20 pr-6 py-6 bg-gray-50 min-h-screen space-y-8 transition-all duration-300 font-sans">
      ...
      {/* GRÁFICO DE LÍNEA PARA EL RIEGO */}
              ...
              <PieChart>
                <Pie 
                  ...
                  label={({ nombre, percent }) => {
                    // Asegura que percent es un número antes de formatear
